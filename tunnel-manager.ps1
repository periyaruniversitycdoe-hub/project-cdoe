...
Write-Host "`n🚀  Starting Cloudflare Tunnels for PhD ERP Portal...`n" -ForegroundColor Cyan
$procs = @()
...
foreach ($svc in $services) {
    ...
    $procs += $proc
    Write-Host "  ▶  $($svc.name) (port $($svc.port)) — PID $($proc.Id)"
    Start-Sleep -Milliseconds 600
}

Write-Host "`n⌛  Waiting 30 seconds for all tunnels to get URLs...`n" -ForegroundColor Yellow
...
foreach ($svc in $services) {
    ...
    if ($content -match 'https://[a-z0-9\-]+\.trycloudflare\.com') {
        ...
        Write-Host "  ✅  $($svc.name.PadRight(15)) $($Matches[0])" -ForegroundColor Green
    } else {
        $urls[$svc.name] = "http://localhost:$($svc.port)"
        Write-Host "  ⚠️   $($svc.name.PadRight(15)) URL not found (fallback localhost)" -ForegroundColor Yellow
    }
}
...
$envMap = @{
    "$root\student\frontend\.env"    = "VITE_STUDENT_API_URL=$studentBe`nVITE_ADMIN_API_URL=$adminBe"
    ...
    "$root\portal-dashboard\.env"    = "VITE_STUDENT_API_URL=$studentBe`nVITE_ADMIN_API_URL=$adminBe`nVITE_SUPERVISOR_API_URL=$supervisorBe`nVITE_CENTER_API_URL=$centerBe"
}
foreach ($pair in $envMap.GetEnumerator()) {
    $pair.Value | Out-File $pair.Key -Encoding utf8 -NoNewline
}
Write-Host "`n📝  Frontend .env files written with tunnel URLs." -ForegroundColor Cyan
...
Write-Host "║  🏛️  Center Portal     : $($urls['center-fe'])" -ForegroundColor White
...
Write-Host @"
╠══════════════════════════════════════════════════════════════════════╣
║  ⚠️  IMPORTANT: Now run restart-frontends.ps1 to reload the .env     ║
║  ⚡  Keep THIS window open — closing it stops all tunnels            ║
║  🛑  Press Ctrl+C to stop all tunnels                                ║
╚══════════════════════════════════════════════════════════════════════╝
"@ -ForegroundColor Green
...
try {
    ...
    Write-Host "`n🛑  Stopping all cloudflared tunnels..."
