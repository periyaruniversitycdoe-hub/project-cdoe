...
const S = {
  ...
  btn: { padding: '11px 28px', background: 'linear-gradient(135deg,#0e7490,#0891b2)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' },
...
export default function Profile() {
  ...
  async function saveProfile(e) {
    ...
    try {
      ...
      setMsg('Profile updated successfully!'); fetchMe();
  ...
  if (!profile) return <div style={{ padding: 40, textAlign: 'center', color: '#0891b2' }}>Loading profile...</div>;

  return (
    <div>
      {/* Centre Profile (read-only) */}
      {profile.centre_name && (
        <div style={S.section}>
          <div style={S.sectionTitle}>Research Centre Profile (Admin Managed)</div>
          <div style={S.row}>
            <div style={S.group}><label style={S.label}>Centre Name</label><div style={S.readOnly}>{profile.centre_name}</div></div>
            <div style={S.group}><label style={S.label}>College code</label><div style={S.readOnly}>{profile.abbreviation || '—'}</div></div>
            <div style={S.group}><label style={S.label}>Centre Ref No.</label><div style={S.readOnly}>{profile.centre_ref_no || '—'}</div></div>
            <div style={S.group}><label style={S.label}>Centre Type</label><div style={S.readOnly}>{profile.centre_type_name || '—'}</div></div>
            <div style={S.group}><label style={S.label}>Centre Email</label><div style={S.readOnly}>{profile.centre_email || '—'}</div></div>
            <div style={S.group}><label style={S.label}>HOD Email</label><div style={S.readOnly}>{profile.hod_email || '—'}</div></div>
            <div style={S.group}><label style={S.label}>District</label><div style={S.readOnly}>{profile.district_name || '—'}</div></div>
            <div style={S.group}><label style={S.label}>Recognition Date</label><div style={S.readOnly}>{profile.recognition_date ? new Date(profile.recognition_date).toLocaleDateString('en-IN') : '—'}</div></div>
          </div>
        </div>
      )}
