...
import { ShieldCheck, ArrowLeft, RefreshCw, GraduationCap } from 'lucide-react';
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 50%, #4338ca 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '36px 32px', boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#1e293b', marginBottom: 6 }}>Verify OTP</h2>
          <p style={{ fontSize: 13, color: '#64748b', marginBottom: 20 }}>
            Enter the 4-digit code sent to <strong style={{ color: '#1e293b' }}>{email}</strong>.
          </p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={loading || timeLeft <= 0}
              style={{ width: '100%', padding: 14, background: (loading || timeLeft <= 0) ? '#a5b4fc' : 'linear-gradient(135deg, #4338ca, #312e81)', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: (loading || timeLeft <= 0) ? 'not-allowed' : 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 14px rgba(67,56,202,0.3)', marginBottom: 20 }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24, fontSize: 13 }}>
              ...
              {resendCooldown > 0 ? (
                <span style={{ color: '#4338ca', fontWeight: '600', fontFamily: 'monospace' }}>Resend in {resendCooldown}s</span>
              ) : (
                <button
                  type="button"
                  onClick={handleResend}
                  disabled={resending}
                  style={{ background: 'none', border: 'none', color: '#4338ca', fontWeight: '700', cursor: 'pointer', padding: 0, fontSize: 13 }}
                >
            ...
            <Link 
              to="/login" 
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, color: '#4338ca', fontWeight: 700, textDecoration: 'none', fontSize: 13 }}
            >
