...
const create = wrap(req => svc.create(req.params.type, req.body));
const update = wrap(req => svc.update(req.params.type, req.params.id, req.body));

const toggleActive = wrap(req => {
    const is_active = req.body.is_active !== undefined ? req.body.is_active : true;
    return svc.toggleActive(req.params.type, req.params.id, is_active);
});

const remove = wrap(async req => {
    await svc.remove(req.params.type, req.params.id);
    return null;
});
