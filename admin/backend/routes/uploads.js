...
router.put('/file-settings/:id', verifyToken, isAdmin, async (req, res) => {
    const { max_size, size_unit, allowed_extensions, is_integrated_course, consolidated_enabled, semester_wise_enabled, max_semesters, allowed_semester_doc_types, per_file_size_limit, total_size_limit } = req.body;
    if (!max_size || !size_unit || !allowed_extensions) {
        return res.status(400).json({ success: false, message: 'max_size, size_unit and allowed_extensions are required' });
    }
    try {
        if (is_integrated_course || req.body.is_integrated_course === 1) {
            await pool.execute(
                `UPDATE file_upload_settings 
                 SET max_size = ?, size_unit = ?, allowed_extensions = ?, 
                     consolidated_enabled = ?, semester_wise_enabled = ?, max_semesters = ?, 
                     allowed_semester_doc_types = ?, per_file_size_limit = ?, total_size_limit = ?
                 WHERE id = ?`,
                [
                    parseInt(max_size, 10), size_unit, allowed_extensions.trim(),
                    consolidated_enabled ? 1 : 0, semester_wise_enabled ? 1 : 0, parseInt(max_semesters, 10) || 10,
                    allowed_semester_doc_types ? allowed_semester_doc_types.trim() : 'jpg,jpeg,png,pdf',
                    parseInt(per_file_size_limit, 10) || 500, parseInt(total_size_limit, 10) || 5000,
                    req.params.id
                ]
            );
        } else {
            await pool.execute(
                'UPDATE file_upload_settings SET max_size = ?, size_unit = ?, allowed_extensions = ? WHERE id = ?',
                [parseInt(max_size, 10), size_unit, allowed_extensions.trim(), req.params.id]
            );
        }
